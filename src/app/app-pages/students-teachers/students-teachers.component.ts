...
import { PupilPrivateInfo, PupilResponse, PupilsParents } from 'src/Interfaces/pupils-models';
import { TeacherPrivateInfo, TeacherResponse, TeachersSubjects } from 'src/Interfaces/teachers-models';
...
export class StudentsTeachersComponent implements OnInit, OnDestroy {
 ...
 displayedTeacherColumns: string[] = ['position', 'name', 'class'];
 displayedPupilColumns: string[] = ['position', 'name', 'phone', 'delete'];
 teacherSubjectsList!: TeachersSubjects[];
 pupilParentsList!: PupilsParents[];
 ...
 isNoTeachersinApp(): boolean {
    if (!this.generatedUser && this.role === 'teachers') {
      return true;
    }
    return false;
 }
      ...
      this.subscription.add(
        this.pupilsRequestService.getPupilInfo(this.generatedUser.id)
          .subscribe(response => {
            ...
            this.pupilParentsList = response.parents;
      ...
      this.subscription.add(
        this.teachersRequestService.getTeacherInfo(this.generatedUser.id)
          .subscribe(response => {
            ...
            this.teacherSubjectsList = response.courses;
            this.patchUserPrivateInfo(response);
            console.log(response);
          },
            (error: HttpErrorResponse) => {
